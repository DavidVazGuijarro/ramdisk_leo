...
busybox mknod -m 600 ${BOOTREC_EVENT_NODEP}
...
if [ -e /cache/recovery/boot ] || busybox grep -q warmboot=0x77665502 /proc/cmdline || busybox grep -q warmboot=0x6f656d01 /proc/cmdline || busybox grep -q warmboot=0x6f656d02 /proc/cmdline || busybox grep -q warmboot=0x6f656d03 /proc/cmdline ; then
...
busybox cat ${BOOTREC_EVENTP} > /dev/keycheck&
busybox sleep 3
...
# boot decision
if [ -s /dev/keycheck ] || busybox grep -q warmboot=0x77665502 /proc/cmdline || busybox grep -q warmboot=0x6f656d01 /proc/cmdline || busybox grep -q warmboot=0x6f656d02 /proc/cmdline || busybox grep -q warmboot=0x6f656d03 /proc/cmdline ; then
	...
	if [ -e /cache/recovery/boot ] || busybox grep -q warmboot=0x77665502 /proc/cmdline || busybox grep -q warmboot=0x6f656d01 /proc/cmdline || busybox grep -q warmboot=0x6f656d02 /proc/cmdline || busybox grep -q warmboot=0x6f656d03 /proc/cmdline ;  then
		...
		## DooMLoRD: handle multiple recovery ramdisks based on bootflag
		busybox echo 'checking CWM flag' >>boot.txt
		if [ -e /cache/recovery/boot_cwm ] || busybox grep -q warmboot=0x6f656d01 /proc/cmdline ; then
		...
		if [ -e /cache/recovery/boot_twrp ] || busybox grep -q warmboot=0x6f656d02 /proc/cmdline ; then
		...
		if [ -e /cache/recovery/boot_philz ] || busybox grep -q warmboot=0x6f656d03 /proc/cmdline ; then
